...
from dishdns_reacon.lib.bingenum import *
from dishdns_reacon.lib.crtenum import scrape_crtsh
from dishdns_reacon.lib.dnshelper import DnsHelper
from dishdns_reacon.lib.shodan import ShodanClientError, make_shodan_client
from dishdns_reacon.lib.whois import *
from dishdns_reacon.lib.yandexenum import *

__version__ = '1.6.0'
__author__ = 'Dishanth, dishanth@hemisx.com'
__name__ = 'cli'

__doc__ = """
DishDNS-Reacon https://github.com/dishanthca/DishDNS-Reacon
...
def shodan_expand_netranges(
    ...
    that fit dishdns_reacon's output format. Results are constrained to the target domain
    and its subdomains to keep the output aligned with dishdns_reacon's domain-centric flow.
    """
    found_records = []
    seen_records = set()
...
def dns_record_from_dict(record_dict_list, scan_info, domains):
    ...
    """
    xml_doc = Element('dishdns_reacon')
...
def main():
    ...
    try:
        ...
        parser.add_argument('-V', '--version', help='DishDNS-Reacon version', action='store_true')
...
Possible types:
    ...
    zonewalk: Perform a DNSSEC zone walk using NSEC records.""",
        ...
        logger.add(Path(Path.home(), '.config/dishdns_reacon/dishdns_reacon.log'), rotation='100 MB', compression='tar.gz')
    ...
    if arguments.version:
        logger.info(f'DishDNS-Reacon version {__version__} https://github.com/dishanthca/DishDNS-Reacon')
        sys.exit(0)
    ...
    if any(dictionary_required):
        ...
        dictionaries = ['/etc/dishdns_reacon/namelist.txt', str(DATA_DIR / 'namelist.txt')]
