...
from dishdns_reacon import cli


def test_check_wildcard():
    with patch('dishdns_reacon.lib.dnshelper.DnsHelper') as mock_dns_helper:
...
def test_brute_domain():
    with patch('dishdns_reacon.lib.dnshelper.DnsHelper') as mock_dns_helper, \
        ...
        with patch('dishdns_reacon.cli.generate_testname', return_value='testname.zonetransfer.me'):
...
def test_brute_domain_uses_provided_dictionary_path(tmp_path):
    ...
    with patch('dishdns_reacon.cli.check_wildcard', return_value=set()):
...
def test_get_nsec_type():
    with patch('dishdns_reacon.lib.dnshelper.DnsHelper') as mock_dns_helper:
        ...
        with patch('dishdns_reacon.cli.get_a_answer', return_value=mock_answer):
...
def test_shodan_expand_netranges_passive_and_active_filtering():
    ...
    with patch('dishdns_reacon.cli.shodan_search_net', return_value=shodan_matches):
