...
**DishDNS-Reacon** is an advanced DNS Enumeration & Reconnaissance tool by **Dishanth**.
...
## Requirements
DishDNS-Reacon requires Python 3.12 or higher.
...
4. Run DishDNS-Reacon:
   ```bash
   uv run dishdns_reacon
   ```
...
DishDNS-Reacon can use Shodan to expand netblocks discovered during standard enumeration from SPF (`-s`) and/or WHOIS (`-w`) data.
...
```bash
uv run dishdns_reacon -d example.com -t std -s -w --shodan --shodan-key "$SHODAN_API_KEY"
```

Active validation of Shodan results (re-resolves hosts and confirms they still match the queried netblock):

```bash
uv run dishdns_reacon -d example.com -t std -s -w --shodan --shodan-active --shodan-key "$SHODAN_API_KEY"
```
...
uv run dishdns_reacon -d example.com -t std -s -w --shodan
```
...
Built by **Dishanth** — [github.com/dishanthca/DishDNS-Reacon](https://github.com/dishanthca/DishDNS-Reacon)
